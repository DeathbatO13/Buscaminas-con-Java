...
public class Juego {
    ...
    private void aumentarMinasCircundantesAlRededor(Posicion p){
        byte fila = p.getFila();
        byte columna = p.getColumna();
        for (int i = fila - 1; i <= fila + 1; i++) {
            for (int j = columna - 1; j <= columna + 1; j++) {
                if (this.tablero.esPosicionValida((byte)i, (byte)j)) {
                    Celda celdaCirc = this.tablero.getCelda((byte)i, (byte)j);
                    celdaCirc.setBombasCircundantes((byte)(celdaCirc.getBombasCircundantes() + 1));
                }
            }
        }
    }
    ...
    private void repartirMinas(){
        ...
        while(bombtemp > 0) {
            ...
            Celda celda = this.tablero.getCelda((byte)randomFila, (byte)randomColumna);
            // Only assign bomb if the cell does not have one
            if (!celda.tieneBomba()) {
                this.tablero.asignarBomba(true, (byte)randomFila, (byte)randomColumna);
    ...
    public Celda.EstadoCelda getEstadoCelda(Posicion p){
        ...
        // Return OCULTA for invalid positions (safe fallback)
        return Celda.EstadoCelda.OCULTA;
    }
	...
	public void descubrir(Posicion p){
        ...
        byte columna = p.getColumna();
        // Validate position
        if (!this.tablero.esPosicionValida(fila, columna)) {
            return;
        }
        Celda celda = this.tablero.getCelda(fila, columna);
        if (celda.getEstado() != EstadoCelda.OCULTA) {
            return;
        }
        // Tiene bomba
        if (celda.tieneBomba()) {
            celda.setEstado(EstadoCelda.DESCUBIERTA);
            this.estado = EstadoJuego.PERDIDO;
            this.descubiertas++;
        }
        // Tiene bombas circundantes
        else if (celda.getBombasCircundantes() > 0) {
            celda.setEstado(EstadoCelda.DESCUBIERTA);
            this.descubiertas++;
        }
        // No tiene minas circundantes
            ...
            for (int i = fila - 1; i <= fila + 1; i++) {
                for (int j = columna - 1; j <= columna + 1; j++) {
                    if (this.tablero.esPosicionValida((byte)i, (byte)j)) {
                        Celda circCelda = this.tablero.getCelda((byte)i, (byte)j);
                        if (circCelda.getEstado() == EstadoCelda.OCULTA) {
                            if (!circCelda.tieneBomba()) {
                                if (circCelda.getBombasCircundantes() == 0) {
                                    descubrir(new Posicion((byte)i, (byte)j));
                                } else {
                                    circCelda.setEstado(EstadoCelda.DESCUBIERTA);
                                    this.descubiertas++;
                                }
                            }
                        }
